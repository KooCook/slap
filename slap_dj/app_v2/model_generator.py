...
from app_v2.models import (Song, Artist, SpotifySong,
                           SpotifyGenre, ArtistSong, YoutubeVideo)
...
def insert_song_from_model(song_model: SongModel) -> 'Song':
    ...
    if len(songs) > 0:
        return songs[0]
    inst.save()
    upsert(SpotifySong, track_id=song_model.spotify_id,
    ...
    for genre_label in song_model.genres:
        try:
            genre = SpotifyGenre.objects.get(name=genre_label)
        except ObjectDoesNotExist:
            genre = SpotifyGenre(name=genre_label)
            genre.save()
        inst.genres.add(genre)
    for idx, artist in enumerate(artists):
        ArtistSong(artist=artist, song=inst, role=ArtistSong.Role.Primary.value if idx == 0 else ArtistSong.Role.Secondary.value).save()
    return inst
